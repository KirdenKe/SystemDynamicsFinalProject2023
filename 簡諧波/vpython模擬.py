...
def ODE(x, t, m1, m2, b1, b2, k1, k2, F):
    ...
    aint = int(a)
    residual = a - aint
    F = np.sin(residual * 2 * np.pi)
    dF = Posmag * (2 * np.pi * frequence) * np.cos(residual * 2 * np.pi)
    dx = x[1];
    dx2 = x[2];
    dx3 = x[3]
    dx4 = (b2 * dF + k2 * F - (m2 * (b1 + b2) + m1 * b2) * x[3] - (m2 * (k1 + k2) + b1 * b2 + m1 * k2) * x[2] - (
                b1 * k2 + b2 * k1) * x[1] - k1 * k2 * x[0]) / (m1 * m2)
...
def ODE2(x, t, m1, m2, b1, b2, k1, k2, F):
    ...
    F = np.sin(residual * 2 * np.pi)
    dF = Posmag * (2 * np.pi * frequence) * np.cos(residual * 2 * np.pi)
    dF2 = -Posmag * np.power((2 * np.pi * frequence), 2) * np.sin(residual * 2 * np.pi)
    dx = x[1];
    dx2 = x[2];
    dx3 = x[3]
    dx4 = (m1 * dF2 + (b1 + b2) * dF + (k1 + k2) * F - (m2 * (b1 + b2) + m1 * b2) * x[3] - (m2 * (k1 + k2) + b1 * b2 + m1 * k2) * x[2] - (
            b1 * k2 + b2 * k1) * x[1] - k1 * k2 * x[0]) / (m1 * m2)
    return [dx, dx2, dx3, dx4]
...
for i in range(0, t.size):
    ...
    a_int = int(a)
    residual = a - a_int
    harmonic[i] = np.sin(residual * 2 * np.pi)
    time += dt
...
x_initial = [0, 0, 0, 0]
x1 = odeint(ODE, x_initial, t, args = (m1, m2, b1, b2, k1, k2, harmonic))

x_initial = [0, 0, 0, 0]
x2 = odeint(ODE2, x_initial, t, args = (m1, m2, b1, b2, k1, k2, harmonic))
